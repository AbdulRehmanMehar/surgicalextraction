<template>
    <div>
        ...
        <div class="align">
            ...
            <div class="grid">
                <h1>Welcome back.</h1>
                <p>Enter your credentials to login or
                    <router-link :to="{name: 'register'}">Register here</router-link>.
                </p>
                <form action="#" method="POST" class="form login" @submit.prevent="login">

                <div class="form__field">
                    <label for="login__email"><i class="icon fas fa-envelope"></i><span class="hidden">Email</span></label>
                    <input id="login__email" v-model="email" type="email" name="username" class="form__input" placeholder="Email" required autofocus>
                </div>
...
export default {
    ...
    methods: {
        login: function() {
            ...
            .then(resp => {
                this.$toastr('success', 'Login was successfull.', 'Success!')
                this.$router.push('/')
            })
            .catch(error => {
                this.errors = []
